...
const CheckoutPage = () => {
  ...
  // Lock ref only after success so it retries if tenantId resolves after first render
  ...
  useEffect(() => {
    ...
    if (cupomParam && items.length > 0 && !coupon && !couponAppliedRef.current) {
      ...
      applyCoupon(cupomParam).then((ok) => { if (ok) couponAppliedRef.current = true; });
    }
                    ...
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="flex items-center gap-1">
                          <button onClick={() => setComboQuantity(comboQuantity - 1)} disabled={comboQuantity <= 1} className="rounded border border-border p-1 hover:bg-muted disabled:opacity-30"><Minus className="h-3.5 w-3.5" /></button>
                          <span className="w-6 text-center text-sm font-semibold">{comboQuantity}</span>
                          <button onClick={() => setComboQuantity(comboQuantity + 1)} className="rounded border border-border p-1 hover:bg-muted"><Plus className="h-3.5 w-3.5" /></button>
                        </div>
                        <button onClick={removeCombo} className="rounded p-1.5 text-destructive hover:bg-destructive/10" title="Remover Combo"><Trash2 className="h-4 w-4" /></button>
                      </div>
                    </div>
