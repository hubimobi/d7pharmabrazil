...
serve(async (req) => {
  ...
  try {
    ...
    // Image generation still uses Lovable AI (Gemini image model) as external LLMs don't support image gen
    ...
    const { error: uploadError } = await supabaseClient.storage.from("product-images").upload(fileName, bytes, { contentType: "image/png" });
    ...
    return new Response(JSON.stringify({ success: true, imageUrl: publicUrl.publicUrl || imageUrl }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (e) {
