...
-- 1. Remove abandoned_carts from realtime
ALTER PUBLICATION supabase_realtime DROP TABLE public.abandoned_carts;

-- 2. Drop public SELECT on ai_agents (exposes system_prompt)
DROP POLICY IF EXISTS "Public can view active agents" ON public.ai_agents;

-- 3. Fix coupons: use correct enum value 'representative'
DROP POLICY IF EXISTS "Public can view active coupons" ON public.coupons;
CREATE POLICY "Authenticated staff can view coupons" ON public.coupons FOR SELECT TO authenticated USING (
  (active = true) AND public.has_any_role(auth.uid(), ARRAY['admin','super_admin','administrador','suporte','gestor','financeiro','representative']::public.app_role[])
);

-- 4. Fix product_testimonials
DROP POLICY IF EXISTS "Anyone can view testimonials" ON public.product_testimonials;
CREATE POLICY "Public can view approved testimonials" ON public.product_testimonials FOR SELECT USING (approved = true);

-- 5. Restricted store_settings view
CREATE OR REPLACE VIEW public.store_settings_public AS
SELECT
  id, store_name, display_name, attendant_name,
  ...
  hide_chat_on_checkout, webchat_enabled,
  ...
  webchat_position, webchat_delay_seconds, webchat_show_on_scroll,
  ...
  checkout_show_motivation, checkout_show_recommendations, checkout_show_testimonials, checkout_show_urgency,
